<template>
  ...
  <div>
    <ul class="infinite-list" v-infinite-scroll="load" style="overflow:auto">
      ...
      <el-row v-for="message in messageList" :key="message.msgId" type="flex"
              :justify="me === message.sender? '' : 'end'">
        <!--    发送发送人  -->
  ...
  data() {
    ...
    changeNowChat(nowChatVal) {
      if (!this.nowChat || this.nowChat.contactId !== nowChatVal.contactId) {
        ...
        let message = this.messageService.getMessagesByContact({
          contact: nowChatVal.contactId,
          skip: 0,
          limit: 10
        }).then(value => {
          if (!value || value.length <= 0) {
            ...
            this.messageList = [
              {
                ...
                sender: 'GiLang',
                msgId: this.socketClient.nextId(),
                contact: nowChatVal.contactId,
                receive: nowChatVal.contactName,
            ...
            // 批量写入
            this.messageService.addMessageBatch(this.messageList)
          } else {
